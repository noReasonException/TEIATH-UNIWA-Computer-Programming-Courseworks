...
int comb(int *arr,int *,int ,int ,int ,int ,struct FilterParameters *ThisFilterParameter);
...
int combination_filter(int *arr,struct FilterParameters *ThisFilterParameter,int flag){
    static int TotalCombinations=0,NonPassFirstFilter=0,NonPassSecondFilter=0,PrintCombinations=0,isFrecuencyTableClear=0;
    static int *FrecuencyTable;
    if(!isFrecuencyTableClear){
        FrecuencyTable = (int *)malloc(sizeof(int)*50);
...
void UserInterface_INPUT(){
    ...
    printf("-----------------------Numbers .....  \n\n");
    struct FilterParameters ThisFilterParameters;
    ThisFilterParameters.x1=x1;
    ThisFilterParameters.x2=x2;
    ThisFilterParameters.y1=y1;
    ThisFilterParameters.y2=y2;
...
int main(int argc,char *argv[]) {
    if(argc<2){printf("No input files!!!");return 0;}
    printf("[WARN] STDIN redirection (file %s )\n",argv[1]);
    FILE *fp = freopen("file.txt","r",stdin);
    if(fp==NULL){
        printf("[WARN] File %s not found (run me as sudo maybe?)\n",argv[1]);
        return 0;
    }
    UserInterface_INPUT();
}